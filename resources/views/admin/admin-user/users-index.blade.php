            ...
            <div class="filter-bar">
                <div class="filter-tabs">
                    ...
                    <a href="{{ route('admin.users.index', ['search' => request('search')]) }}"
                        class="filter-tab {{ !request('role') && !request('status') ? 'active' : '' }}">All Users<span
                            class="filter-count">{{ $allCount }}</span>
                    </a>

                    <a href="{{ route('admin.users.index', ['role' => 'Developer', 'search' => request('search')]) }}"
                        class="filter-tab {{ request('role') === 'Developer' ? 'active' : '' }}">Developer<span
                            class="filter-count">{{ $developerCount }}</span>
                    </a>

                    <a href="{{ route('admin.users.index', ['role' => 'Admin', 'search' => request('search')]) }}"
                        class="filter-tab {{ request('role') === 'Admin' ? 'active' : '' }}">Admin<span
                            class="filter-count">{{ $adminCount }}</span>
                    </a>

                    <a href="{{ route('admin.users.index', ['status' => 'Active', 'search' => request('search')]) }}"
                        class="filter-tab {{ request('status') === 'Active' ? 'active' : '' }}">Active<span
                            class="filter-count">{{ $activeCount }}</span>
                    </a>

                    <a href="{{ route('admin.users.index', ['status' => 'Inactive', 'search' => request('search')]) }}"
                        class="filter-tab {{ request('status') === 'Inactive' ? 'active' : '' }}">Inactive<span
                            class="filter-count">{{ $inactiveCount }}</span>
                    </a>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="col-status">
                                        <span
                                            class="status-badge {{ $user->status === 'Active' ? 'status-active' : 'status-inactive' }}">{{ $user->status }}</span>
                                        <a href="{{ route('admin.users.show', $user) }}" class="icon-btn"
                                            title="View User">
                                            <svg viewBox="0 0 24 24">
                                                <path
                    ...
                    <div class="pagination">
                        ...
                        @for ($page = 1; $page <= $users->lastPage(); $page++)
                            <a href="{{ $users->url($page) }}"
                                class="page-btn {{ $users->currentPage() === $page ? 'active' : '' }}">{{ $page }}</a>
                        @endfor
